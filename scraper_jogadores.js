...
async function upsertJogadores(plantel, idEquipaDB) {
    ...
    const jogadores = plantel.map(j => ({
        ...
        avatar_url: j.avatar_url, // 🚀 Gravação da foto
        ...
        try {
            ...
            while (temProximaPagina) {
                ...
                for (const eq of linhasEquipas) {
                    ...
                    // 🚀 EXTRAÇÃO DA FOTO ADAPTADA PARA IDs NUMÉRICOS
                        ...
                        grids.forEach(table => {
                            ...
                            table.querySelectorAll('tbody tr').forEach(tr => {
                                ...
                                if (tds.length > 2) {
                                    ...
                                    // 1. Tentar encontrar uma tag <img> óbvia
                                    const imgTag = tr.querySelector('img');
                                    if (imgTag) {
                                        const src = imgTag.getAttribute('src');
                                        if (src && !src.toLowerCase().includes('nopicture') && !src.toLowerCase().includes('default')) {
                                            fotoUrl = src.startsWith('http') ? src : `https://fpp.tiepadel.com${src}`;
                                        }
                                    }

                                    // 2. FORÇA BRUTA: Extrair ID ou Slug do link Oculto
                                    if (!fotoUrl) {
                                        ...
                                        if (linkMatch && linkMatch[1]) {
                                            ...
                                            // Caso A: Dashboard.aspx?id=416599
                                            if (href.includes('?id=')) {
                                                playerId = href.split('?id=')[1].split('&')[0];
                                            }
                                            // Caso B: /player/Tiagomorgado
                                            else if (href.includes('/player/')) {
                                                playerId = href.split('/player/')[1].split('/')[0].split('?')[0];
                                            }

                                            // Se apanhou um ID, gera o link da foto!
                                            if (playerId) {
                                                fotoUrl = `https://fpp.tiepadel.com/Handlers/ShowImage.ashx?id=${playerId}&type=Player`;
                                            }
                                        }
                                    }
                    ...
                    if (plantel.length > 0) {
                        const comFoto = plantel.filter(j => j.avatar_url).length;
                        console.log(`      📸 Fotos associadas: ${comFoto} de ${plantel.length} jogadores.`);
