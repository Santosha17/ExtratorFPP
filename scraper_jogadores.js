                ...
                for (const eq of linhasEquipas) {
                    ...
                    await page.evaluate(() => {
                        document.querySelectorAll('table.rgMasterTable').forEach(t => {
                            if (t.textContent.toUpperCase().includes('LICEN')) t.remove();
                        });
                    });

                    // Prepara a promessa para esperar pela resposta AJAX antes de clicar
                    ...
                    try {
                        ...
                        // Aguarda 1.5s para o JavaScript da página processar a resposta e desenhar a tabela
                        await new Promise(r => setTimeout(r, 1500));
                    } catch (e) {
                        console.log(`   ⚠️ Timeout a carregar jogadores da equipa ${eq.nomeEquipa}`);
                        continue;
                    }

                    // 1. Extrair os dados base
                    const plantelBase = await page.evaluate((equipaEsperada) => {
                        const resultados = [];
